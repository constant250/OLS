    ...
    <div class="row">
      <div class="col-md-12">
        <div class="position-relative">
          <form ref="fileform" class="file-form mb-4 mt-0 pt-4 px-2" :class="[onDrag ? 'on-drag' : '']">
              ...
              <div class="card-wrapper col-3 mb-4 position-relative" v-for="(file, key) in sortDescFiles">
                <a v-bind:href="previewURL+file.id" class="card file-listing h-100" v-bind:key='file.id' target="_blank">
                  <div class="img-icon-container">
                    <div v-if="$props.versioning === true" class="extra-details">
                      <div v-if="file.is_current === 1" class="">Current</div>
                      <div class="current-file">v{{ file.version  }}</div>
                    </div>
            ...
            <div class="fileform-caption"><i class="fas fa-file-upload"></i> Drop the files here!</div>
            <div class="progress" :class="[progressBar ? 'show' : '']">
              <!-- <div class="progress-bar" role="progressbar" v-bind:style.prop="'width: 50%'" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100">50%</div> -->
          ...
          <!-- <div class="fileform-caption  fixed-top h-100 w-100 d-flex align-items-center justify-content-center">
            <div class="text-center w-50">
              <i class="fas fa-file-upload"></i>
              <span class="drop-files d-block mb-2">Drop the files here!</span>
              <div class="progress">
                <div class="progress-bar" role="progressbar" v-bind:style.prop="'width: ' +uploadPercentage+ '%'" v-bind:aria-valuenow.prop="uploadPercentage" aria-valuemin="0" aria-valuemax="100">{{ uploadPercentage }}%</div>
              </div>
            </div>
           ...
           if (this.fileDetails === true) {
                for( let i = 0; i < e.dataTransfer.files.length; i++ ){
                  ...
                  for(let c = 0; c < this.files.length; c++) {
                    ...
                    if(this.versioning != true){
                      if(dataFiles.name == this.files[c].name) {
                        ...
                        Toast.fire({
                            position: 'top-end',
                            type: 'error', title: 'File already exist! Please click update file to add new version of this file.',
                        })
                        break;
                      }
                    }
                  ...
                  if(compareName == 1) {
                      Toast.fire({
                        position: 'top-end',
                        type: 'error', title: 'File is not the same! Please refer to the name and file type.',
                      })
                      break;
                  } else {
                    if (compareID != 1 ) {
      ...
      */
        ...
        axios.post( this.uploadURL,
          formData,
          {
            ...
            if(res.data.status == 'success'){
              Toast.fire({
                  position: 'top-end',
                  type: 'success', title: 'Attachment saved successfully',
              })
              if(vm.versioning){
                vm.fetchAttachments();
              } else {
                vmFile.push(res.data.file);
              }
              // console.log(vmFile);
            // console.log('SUCCESS!!');
              ...
              Toast.fire({
                  position: 'top-end',
                  type: 'error', title: 'Attachment was not saved successfully',
              })
            }
        })
        .catch(function(){
      ...
      removeFile( file, key ) {
        ...
        swal.fire({
          ...
          text: file.name+" won't be able to revert!",
          icon: 'warning',
          showCancelButton: true,
          ...
          if (result.value) {
            axios.get(vm.deleteURL+'/'+vmFile.id)
            .then(function(res){
              // vm.files = res.data;
              if(res.data.status == 'success'){
      ...
      updateNote (id) {
        ...
        swal.fire({
          ...
          html:
          '<input id="note" class="swal2-input">' +
          '<div class="custom-control custom-checkbox">' +
            '<input type="checkbox" class="custom-control-input" id="is_current">' +
            '<label class="custom-control-label" for="is_current">Check this to set file as current.</label>' +
          '</div>',
          showCancelButton: true,
...
.file-form{
  display: block;min-height: 200px;height: auto;background: #ffffff;border: 2px solid #eaeaea;margin: auto;text-align: center;border-radius: 4px;border-top-left-radius: 0 !important;
  &:hover,
  &:focus,
  &:active,
  &.on-drag{
    background: rgba(78, 115, 223, 0.2);border: 2px solid #4e73df;
  }
  .fileform-caption{
    display: none;user-select: none;pointer-events: none;color: #ffffff;position: absolute;top: -26px;left: 0;background: #4e73df;padding: 5px 12px;font-size: 12px;border-top-left-radius: 4px;border-top-right-radius: 4px;
  }
  ...
  .file-listing {
    &:hover {
      text-decoration: none;box-shadow: 0px 0px 10px 1px rgba(78,115,223,0.5);-webkit-box-shadow: 0px 0px 10px 1px rgba(78,115,223,0.5);-moz-box-shadow: 0px 0px 10px 1px rgba(78,115,223,0.5);
      .preview-item-img{
  ...
  .card {
    ...
    .img-icon-container{
      ...
      .preview-item-icon{
        ...
        position: relative;height: 100%; text-align: center;vertical-align: middle;font-weight: bolder;color: #fff;border-top-left-radius: 4px;border-top-right-radius: 4px;font-size: 24px;display: flex;justify-content: center;align-items: center;
        label::before {
  ...
  .progress{
    display: none;background-color: transparent;position: absolute;top: 0;left: 0;width: 100%;border-radius: 0;
    &.show{
      display: block;
    }
  }
  .card-btn{
    position: absolute;top: 0;right: .75rem;color: #FFFFFF !important;z-index: 9;
    .update{
      &:hover{
